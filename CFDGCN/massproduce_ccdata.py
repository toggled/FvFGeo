...
# def getDSDF(pos, bd, theta_rot, theta_seg, inf=5):
    ...
    # return dSDF
# ,pos, bd theta_rot, theta_seg, inf=5
def getDSDF(pos, bd, theta_rot, theta_seg, inf=5):
# def getDSDF(data, theta_rot, theta_seg, inf=5):
    '''get dSDF representation of geometry given rotation angle, segment angle.'''
    ...
    def sameSide(j,i):
        '''Check if boundary points j (Mx2) and all points i (Nx2)
        fall on the same side of the geometry (bool MxN)'''
        c = torch.mean(j,dim=0).unsqueeze(0) #airfoil internal point
        j = order_clockwise(j); j1 = torch.cat([j[1:,:],j[0:1,:]])
        indi, indj = torch.meshgrid(torch.arange(i.size(0)), \
                                  torch.arange(j.size(0)), indexing='ij')
        p0 = i[indi]; p1 = j[indj]; p2 = j1[indj]
        ...
        dir_c = (j[:,1]-c[:,1])*(j1[:,0]-j[:,0])
        dir_c -= (j1[:,1]-j[:,1])*(j[:,0]-c[:,0])
        side = ((dir_i>0)==(dir_c<=0)).t() #bool size MxN
        return j, side.unsqueeze(2)
    def PDF(mean,x):
        '''chosen PDF to used to weigh points'''
        return torch.ones(x.size()).to(x.device) #uniform distribution
    def intPDF(mean,mRange):
        '''integral of chosen PDF used to weigh points'''
        return (mRange[1,:,:]-mRange[0,:,:]) #for uniform distribution


    j = pos[bd,:]; j, side = sameSide(j,pos)

    #compute inverse distance di from every point i to every bd point
    h = j[:,0:1] - pos.t()[0:1,:] #size MxN
    k = j[:,1:] - pos.t()[1:,:] #size MxN
    d = torch.sqrt(k*k+h*h).unsqueeze(2); d[d>inf]=inf #if dSDF. #size MxN
    # di = 1/(1+d) #inverse distance. if d=inf then di=0

    #Compute dSDF for ever segment centre theta_cen and segment range theta_ran
    theta_cen = torch.arange(0,2*torch.pi,theta_rot) #size Sn
    theta_ran = torch.stack([theta_cen-(theta_seg/2), theta_cen+(theta_seg/2)]) # 2xSn

    #compute dSDF_i for every node i in V
    theta_ij = torch.atan2(k,h) #size MxN -> MxNxSn
    theta_ij = theta_ij.unsqueeze(2).expand(*theta_ij.size(),theta_cen.size(0)).clone()
    theta_ij[:,:,1:] = theta_ij[:,:,1:]%(2*torch.pi) #if theta_ran[0]>=0 else theta_ij

    #estimate integral as weighted average within segment range AND same side
    tr0 = theta_ran[0:1,:].unsqueeze(0).expand(*theta_ij.size()).to(theta_ij.get_device())
    tr1 = theta_ran[1:,:].unsqueeze(0).expand(*theta_ij.size()).to(theta_ij.get_device())
    # print(tr0.get_device())
    # print(tr1.get_device())
    # print(theta_ij.get_device())
    ind = (tr0<=theta_ij)*(theta_ij<=tr1)*side #size MxNxSn
    w = PDF(theta_cen,theta_ij) #size MxNxSn
    dSDF_i = torch.sum((d*w)*ind,dim=0) #size NxSn
    w = torch.sum((w*ind),dim=0) #sum of discrete weights, size NxSn
    dSDF_i = torch.nan_to_num(dSDF_i/w,nan=0) #nan=0 for dSDF.

    #minimum angle range from point i to point j
    theta_minR = torch.zeros(2,*w.size()).to(dSDF_i.device) # 2xNxSn
    ind = (tr0<=theta_ij)*(theta_ij<=tr1) #in segment range (but any side)
    theta_ij[~ind] = 2*torch.pi; theta_minR[0,:,:] = torch.min(theta_ij,dim=0)[0] #NxSn
    theta_ij[~ind] = -2*torch.pi; theta_minR[1,:,:] = torch.max(theta_ij,dim=0)[0] #NxSn

    #compute weight of minimum angle segment w_tMin
    w_tMin = intPDF(theta_cen,theta_minR)/intPDF(theta_cen,theta_ran.unsqueeze(1))
    w_tMin[w_tMin<0] = 0
    dSDF = w_tMin*dSDF_i + (1-w_tMin)*inf #since now "inf" is zero
...
if __name__=='__main__':
    ...
    for i,casename in enumerate(tqdm(manifest_all[start:end], desc = device)):
        ...
        if stat_type == 'coarse':
            ...
            internal_vtu = pv.read(vtkpath+s+'/'+s+'_internal.vtu')
        #print(cC.shape,' ',internal_vtu.cell_data['vtkOriginalCellIds'].shape)
        _include = np.array([False]*cC.shape[0])
        _include[internal_vtu.cell_data['vtkOriginalCellIds']] = True
        _,bd_edges = get_edges(internal_vtu)
        markers = compute_Cellmarkers_numpy(internal_vtu,bd_edges)
        ...
        # Fine data
        fvf_time , dt_fine = get_graph_ns(mesh,case,_include, farfieldmarker, cC,V,magSf,Cf,Sf,num_nodes=V.shape[0],\
                    converged_folder = os.path.join(case,_iter),Uinf=Uinf, alpha=alpha, isfine = True)      
        # print(ccgraph_time)
        if stat_type == 'coarse':
            outf = 'cc_graph_LRmesh3.txt'
        else:
            outf = 'cc_graph_finemesh3.txt'
        with open(outf,'a') as f: #coarse
            s_tm = time()
            dt_fine.sdf = GcomputeSDF(dt_fine.x[:,:2],dt_fine.surf)
            sdf_time = time() - s_tm
            s_tm = time()
            ...
            try:
                ...
                raise(e)
            did_time = time() - s_tm
            f.write(str(len(dt_fine.x))+' '+str(sdf_time)+' '+str(fv_time)+' '+str(did_time)+' '+str(fvf_time)+'\n')
