...
def runcase(c):
    if args.fine:
        ...
        #save_vtpvtu(casepath=c,savepath=savepath, save_uncropped_vtp=True)
        save_vtpvtu(casepath=c,savepath=savepath, save_uncropped_vtp=False)
        save_SDFSAFDSDF_data(c,savepath,device = args.device)
...
if __name__=='__main__':
    ...
    input_list = []
    for name in (manifest['full_train']+manifest['full_test']):
