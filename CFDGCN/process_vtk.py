...
def save_vtpvtu(casepath,savepath,save_uncropped_vtp=False):
    ...
    if save_uncropped_vtp:
        aerofoil_vtp = get_airfoil_vtp(boundary,compute_length = True)
        vtpname = casepath.split('/')[-1]+"_aerofoil_uncropped.vtp"
        vtuname = casepath.split('/')[-1]+"_internal_uncropped.vtu"
        subdir = os.path.join(savepath,casepath.split('/')[-1])
        if not os.path.exists(subdir):
            os.system('mkdir -p '+subdir)
        internal_vtu = crop_internal(slice_z_center(internal_mesh), crop =  [-100, 100, -100, 100, 0.5, 0.5]) 
