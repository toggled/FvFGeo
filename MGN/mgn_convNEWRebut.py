...
class EdgeProcessor(nn.Module):
    ...
    def __init__(self, hs:List[int]):
        super().__init__()
        self.edge_mlp = nn.Sequential(MLP(hs=hs, act=nn.ReLU()), nn.LayerNorm(hs[-1]))
...
class NodeProcessor(nn.Module):
    ...
    def __init__(self, hs:List[int]):
        super().__init__()
        self.node_mlp = nn.Sequential(MLP(hs=hs, act=nn.ReLU()), nn.LayerNorm(hs[-1]))
...
class MeshGraphNetsConv(MessagePassing):
    def __init__(self, node_dim:int, edge_dim:int, use_FV=True):
        super().__init__(aggr='add')
        self.node_dim = node_dim
        self.edge_dim = edge_dim
        
        if use_FV:
            self.edge_processor = EdgeProcessor([(node_dim+1)*2 + (edge_dim+6)] + [edge_dim]*3)
            self.node_processor = NodeProcessor([(node_dim+1)   + (edge_dim)] + [node_dim]*3)
        else:
