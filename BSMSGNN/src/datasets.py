...
class ShapeFV(Dataset):
    def __init__(self,
        ...
        # read all features indicated in meta
        with open(os.path.join(root, 'meta_cc.json'), 'r') as fp:
            self.meta = json.loads(fp.read())
        field_names = self.meta['field_names']
        # print('field_names: ',field_names)
        fields = dict()
        h5file = os.path.join(self.data_dir, 'shape'+str(instance_id) + '_cc.h5')
        # print('h5file = ',h5file)
        self.fv = False
...
class MeshGeneralDataset(Dataset):
    def __init__(self,
        ...
        self._cal_multi_mesh(fields)
        in_feature, tar_feature = self._preprocess(fields)
        in_feature = in_feature[0:1,:,:] # EDITED
        tar_feature = tar_feature[0:1,:,:] # EDITED
        # normalization
        self.in_feature, self.tar_feature = self._normalize(in_feature, tar_feature)
        super().__init__(root)
...
class MeshAirfoilDataset(MeshGeneralDataset):
    ...
    def _preprocess(self, fields):
        ...
        # in: vel, density, pos, type
        # out: d_vel, d_density, pressure
        # print('fields["velocity"] = ',fields["velocity"][:-1].shape)
        # print(fields["density"][:-1].shape)
        # print(fields["velocity"][1:].shape)
