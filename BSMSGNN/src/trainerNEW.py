...
class Trainer:
    ...
    def _create_model(self):
        ...
        elif self.args.case.startswith('fvairfoil'):
            self.model_class = modelsNEW.FVAirfoil
            self.dataset_class = cdata.MeshAirfoilDataset
        else:
            raise NotImplementedError("A Case not wrapped yet")
            
        if self.args.case == 'fvairfoil_FV_SAF_dSDF':
            self.model = self.model_class(pos_dim=self.args.space_dim, ld=self.args.hidden_dim,\
                    ...
                    MP_times=self.args.mp_time,use_FV=True,use_SAF=True,use_dSDF=True,use_res=False)
        elif self.args.case == 'fvairfoil_SAF_dSDF':
            self.model = self.model_class(pos_dim=self.args.space_dim, ld=self.args.hidden_dim,\
                    layer_num=self.args.multi_mesh_layer, mlp_hidden_layer=self.args.hidden_depth,\
    ...
    def run_epoch(self, epoch, mode='train'):
        ...
        for i in range(len(instance_list)): # Samples
            id = instance_list[i]
            mdata = self._create_datset_offline(id, mode=mode)
            mean_loss = 0
            count = 0
            pen_coeff = mdata.suggested_pen_coef().to(self.device)
            for id_batch, b_data in enumerate(DataLoader(mdata, batch_size=self.args.batch, shuffle=True)): # time
                m_ids, m_gs, b_data = self._preproc_multi_infos(mdata, b_data)
                # optimization
                self.optimizer.zero_grad()
                
                self.args.case == 'fvairfoil':
                    loss, _, non_zero_elements = self.model(m_ids, m_gs, b_data.x, b_data.y, \
                                                        pen_coeff, data=b_data)
                else:
