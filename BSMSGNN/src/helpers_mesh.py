...
def tetras_to_edges(cells):
    ...
    # collect edges from triangles
    t_cell = torch.tensor(cells)
    edge_index = torch.cat((t_cell[:, :2], t_cell[:, 1:3], t_cell[:, 2:4], torch.cat(
        (t_cell[:, 3].unsqueeze(1), t_cell[:, 0].unsqueeze(1)), -1), torch.cat(
            (t_cell[:, 0].unsqueeze(1), t_cell[:, 2].unsqueeze(1)), -1), torch.cat((t_cell[:, 1].unsqueeze(1), t_cell[:, 3].unsqueeze(1)), -1)), 0)
    # those edges are sometimes duplicated (within the mesh) and sometimes
    # single (at the mesh boundary).
    # sort & pack edges as single torch long tensor
    r, _ = torch.min(edge_index, 1, keepdim=True)
...
def quads_to_edges(cells):
    ...
    t_cell = torch.tensor(cells)
    edge_index = torch.cat((t_cell[:, :2], t_cell[:, 1:3], t_cell[:, 2:4], torch.cat((t_cell[:, 3].unsqueeze(1), t_cell[:, 0].unsqueeze(1)), -1)), 0)
    # those edges are sometimes duplicated (within the mesh) and sometimes
    # single (at the mesh boundary).
    # sort & pack edges as single torch long tensor
    r, _ = torch.min(edge_index, 1, keepdim=True)
